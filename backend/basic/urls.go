...
func init() {
	...
	basicRouter := goi_example.Server.Router.Include("basic/", "父路由")
	{
		...
		basicRouter.Path("valid/", "参数验证", goi.ViewSet{POST: TestParamsValid})

		basicRouter.Path("converter/<string:name>", "自定义路由转换器获取参数", goi.ViewSet{GET: TestConverterParamsStr})

		basicRouter.Path("converter_phone/<phone:phone>", "测试 phone 路由转换器", goi.ViewSet{GET: TestConverterParamsPhone})

		basicRouter.Path("context/<string:name>", "从上下文中获取数据", goi.ViewSet{GET: TestContext})

		basicRouter.Path("context_params", "从上下文参数中获取数据", goi.ViewSet{GET: TestContextParams})
		...
		// embed.FS 静态文件
		basicRouter.StaticFileFS("favicon.svg", "测试静态FS文件", web.Favicon)
