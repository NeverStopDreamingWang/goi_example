...
//
// 参数:
//   - payload any: JWT负载信息,会被序列化为 JWT Claims
//   - key string: 用于签名的密钥
//
// 返回:
...
func NewToken(payload any, key string) (string, error) {
...
//   - key string: 验证签名用的密钥
//   - payloadsDest any: 用于存储解码后payload的结构体指针
//
// 返回:
//   - error: 验证过程中的错误，包括:
...
func CheckToken(tokenString string, key string, payloadsDest any) error {
	// 解析并验证 token
